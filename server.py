...
def html_page(title, body):
    return f'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{title}</title><style>body{{font-family:Inter,Arial,sans-serif;background:#f7f1e3;color:#0b1f3a;margin:0}}header{{background:#0b1f3a;color:#fff;padding:18px 28px;display:flex;justify-content:space-between;align-items:center}}main{{max-width:1100px;margin:28px auto;padding:0 18px}}a,button{{font:inherit}}.card{{background:#fff;border-radius:14px;padding:20px;margin:16px 0;box-shadow:0 4px 20px #0b1f3a12}}table{{width:100%;border-collapse:collapse;background:#fff}}th,td{{padding:12px;border-bottom:1px solid #ddd;text-align:left;vertical-align:top}}th{{background:#0b1f3a;color:#fff}}.grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:14px}}.stat{{background:#fff;padding:18px;border-radius:14px}}.stat b{{font-size:28px;display:block}}.btn{{background:#f4a51c;color:#0b1f3a;border:0;border-radius:8px;padding:10px 14px;font-weight:700;cursor:pointer}}input,select{{padding:10px;border:1px solid #ccc;border-radius:7px}}@media(max-width:700px){{.grid{{grid-template-columns:1fr 1fr}}table{{font-size:13px}}}} </style></head><body>{body}</body></html>'''
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path=='/admin-login':
            ...
            body=f'''<header><strong>100% FOCUS CLUB</strong></header><main><div class="card"><h1>Admin Login</h1><p>{err}</p><form method="post" action="/admin-login"><p><input name="username" placeholder="Username" required></p><p><input type="password" name="password" placeholder="Password" required></p><button class="btn">Sign in</button></form></div></main>'''
        ...
        if path=='/admin':
            ...
            c=db(); qs=parse_qs(urlparse(self.path).query); ftype=qs.get('type',[''])[0]; fstatus=qs.get('status',[''])[0]; where=[]; args=[];
            if ftype: where.append('type=?'); args.append(ftype)
            if fstatus: where.append('status=?'); args.append(fstatus)
            sql='SELECT * FROM submissions' + ((' WHERE '+' AND '.join(where)) if where else '') + ' ORDER BY created_at DESC'; rows=c.execute(sql,args).fetchall(); counts={r['type']:c.execute('SELECT COUNT(*) n FROM submissions WHERE type=?',(r['type'],)).fetchone()['n'] for r in c.execute('SELECT DISTINCT type FROM submissions')}; c.close()
            stats=''.join(f'<div class="stat"><span>{t}</span><b>{n}</b></div>' for t,n in counts.items()) or '<div class="stat"><span>Total submissions</span><b>0</b></div>'
            trs=''
            for r in rows:
                trs+=f'<tr><td>{r["reference"]}</td><td>{r["type"]}</td><td>{r["name"] or ""}<br>{r["email"] or ""}</td><td>{r["created_at"][:19].replace("T"," ")}</td><td><select onchange="status({r["id"]},this.value)"><option {"selected" if r["status"]=="New" else ""}>New</option><option {"selected" if r["status"]=="Contacted" else ""}>Contacted</option><option {"selected" if r["status"]=="Scheduled" else ""}>Scheduled</option><option {"selected" if r["status"]=="Completed" else ""}>Completed</option><option {"selected" if r["status"]=="Closed" else ""}>Closed</option></select></td><td><details><summary>View</summary><pre>{r["payload"]}</pre><label>Admin notes</label><textarea id="n{r["id"]}" style="width:100%;min-height:70px">{r["notes"]}</textarea><button class="btn" onclick="saveNotes({r["id"]})">Save notes</button></details></td></tr>'
            body=f'''<header><strong>100% FOCUS CLUB — ADMIN</strong><span><a href="/" style="color:#fff;margin-right:18px">View site</a><a href="/admin-logout" style="color:#fff">Sign out</a></span></header><main><h1>Submissions</h1><div class="grid">{stats}</div><div class="card"><form method="get" style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:16px"><select name="type"><option value="">All types</option><option>School Request</option><option>Youth Registration</option><option>Sponsor / Donor Interest</option><option>Contact Message</option></select><select name="status"><option value="">All statuses</option><option>New</option><option>Contacted</option><option>Scheduled</option><option>Completed</option><option>Closed</option></select><button class="btn">Filter</button></form><table><thead><tr><th>Reference</th><th>Type</th><th>Contact</th><th>Received</th><th>Status</th><th>Details</th></tr></thead><tbody>{trs}</tbody></table></div><script>async function status(id,status){{await fetch('/admin/status',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{id,status}})}});location.reload()}}</script></main>'''
